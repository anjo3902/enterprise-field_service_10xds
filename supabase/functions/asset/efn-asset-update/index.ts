/**
 * asset/efn-asset-update/index.ts
 */
...
import { updateAsset }                    from "./handler.ts";
import { UpdateAssetSchema }              from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body = await parseBody(req, UpdateAssetSchema, correlationId);
    ...
    log.info({ correlationId, asset_pk: result.asset_id_pk, duration_ms: log.elapsed() }, "Asset update complete");
