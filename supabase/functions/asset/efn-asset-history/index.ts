/**
 * asset/efn-asset-history/index.ts
 */
...
import { getAssetHistory }                from "./handler.ts";
import { GetAssetHistorySchema }          from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, ["system_admin", "org_admin", "org_manager", "vendor_admin", "vendor_supervisor", "technician", "read_only"], correlationId);
    const body = await parseBody(req, GetAssetHistorySchema, correlationId);
