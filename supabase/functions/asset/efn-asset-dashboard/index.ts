/**
 * asset/efn-asset-dashboard/index.ts
 */
...
import { getAssetDashboard }              from "./handler.ts";
import { AssetDashboardSchema }           from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, ["system_admin", "org_admin", "org_manager", "read_only"], correlationId);
    const body = await parseBody(req, AssetDashboardSchema, correlationId);
