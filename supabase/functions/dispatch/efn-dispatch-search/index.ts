/**
 * dispatch/efn-dispatch-search/index.ts
 */
...
import { searchDispatches }               from "./handler.ts";
import { DispatchSearchSchema }           from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body = await parseBody(req, DispatchSearchSchema, correlationId);

    const result = await searchDispatches(body, ctx.claims, correlationId);
