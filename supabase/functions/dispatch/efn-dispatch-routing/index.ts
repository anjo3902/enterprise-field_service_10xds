/**
 * dispatch/efn-dispatch-routing/index.ts
 */
...
import { calculateRouting }               from "./handler.ts";
import { DispatchRoutingSchema }          from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, [
      ...
      "dispatcher", "vendor_admin", "vendor_supervisor", "support_engineer", "platform_operator"
    ], correlationId);
    const body = await parseBody(req, DispatchRoutingSchema, correlationId);

    const result = await calculateRouting(
