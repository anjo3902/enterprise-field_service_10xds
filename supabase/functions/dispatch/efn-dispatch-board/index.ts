/**
 * dispatch/efn-dispatch-board/index.ts
 */
...
import { getDispatchBoard }               from "./handler.ts";
import { DispatchBoardSchema }            from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body = await parseBody(req, DispatchBoardSchema, correlationId);

    const result = await getDispatchBoard(body, ctx.claims, correlationId);
