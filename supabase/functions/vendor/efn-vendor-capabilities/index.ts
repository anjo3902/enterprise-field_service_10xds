/**
 * vendor/efn-vendor-capabilities/index.ts
 *
 * POST /vendor/capabilities
 * Body: { action: "upsert"|"remove", vendor_id, ... }
...
import { handleCapability }               from "./handler.ts";
import { CapabilityActionSchema }         from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body = await parseBody(req, CapabilityActionSchema, correlationId);
    ...
    log.info({ correlationId, vendor_id: result.vendor_id, duration_ms: log.elapsed() }, "Capability action complete");
