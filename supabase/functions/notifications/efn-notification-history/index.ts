/**
 * notifications/efn-notification-history/index.ts
 */
...
import { getHistory }                     from "./handler.ts";
import { HistorySchema }                  from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Any authenticated user can access their own history
    ...
    const body = await parseBody(req, HistorySchema, correlationId);

    const result = await getHistory(body, ctx.claims, correlationId);
