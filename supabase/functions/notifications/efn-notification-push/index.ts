/**
 * notifications/efn-notification-push/index.ts
 */
...
import { handlePushDelivery }             from "./handler.ts";
import { PushDeliverySchema }             from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body = await parseBody(req, PushDeliverySchema, correlationId);

    const result = await handlePushDelivery(body, ctx.claims, correlationId);
