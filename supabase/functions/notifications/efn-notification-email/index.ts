/**
 * notifications/efn-notification-email/index.ts
 */
...
import { handleEmailDelivery }            from "./handler.ts";
import { EmailDeliverySchema }            from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body = await parseBody(req, EmailDeliverySchema, correlationId);

    const result = await handleEmailDelivery(body, ctx.claims, correlationId);
