...
import { handleAiDiagnosis }              from "./handler.ts";
import { AiDiagnosisSchema }              from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, [
      "system_admin", "org_admin", "org_manager", "dispatcher", "maintenance_manager", "platform_operator"
    ], correlationId);
    
    const body = await parseBody(req, AiDiagnosisSchema, correlationId);

    const result = await handleAiDiagnosis(body, ctx.claims, correlationId);
