/**
 * ai/efn-ai-maintenance/index.ts
 */
...
import { handleAiMaintenance }            from "./handler.ts";
import { AiMaintenanceSchema }            from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, [
      "system_admin", "org_admin", "org_manager", "maintenance_manager", "platform_operator"
    ], correlationId);
    
    const body = await parseBody(req, AiMaintenanceSchema, correlationId);

    const result = await handleAiMaintenance(body, ctx.claims, correlationId);
