...
import { handleHitlReview }               from "./handler.ts";
import { HitlSchema }                     from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, [
      "system_admin", "org_admin", "org_manager", "dispatcher", "maintenance_manager", "ai_reviewer"
    ], correlationId);
    
    const body = await parseBody(req, HitlSchema, correlationId);

    const result = await handleHitlReview(body, ctx.claims, correlationId);
