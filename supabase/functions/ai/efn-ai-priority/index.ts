...
import { handleAiPriority }               from "./handler.ts";
import { AiPrioritySchema }               from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body = await parseBody(req, AiPrioritySchema, correlationId);

    const result = await handleAiPriority(body, ctx.claims, correlationId);
