...
import { handleAiDispatch }               from "./handler.ts";
import { AiDispatchSchema }               from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, [
      "system_admin", "org_admin", "org_manager", "dispatcher", "platform_operator"
    ], correlationId);
    
    const body = await parseBody(req, AiDispatchSchema, correlationId);

    const result = await handleAiDispatch(body, ctx.claims, correlationId);
