/**
 * technician/efn-tech-skills/index.ts
 */
...
import { handleTechSkill }                from "./handler.ts";
import { TechSkillActionSchema }          from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, ["system_admin", "vendor_admin", "vendor_supervisor"], correlationId);
    const body = await parseBody(req, TechSkillActionSchema, correlationId);

    const result = await handleTechSkill(
