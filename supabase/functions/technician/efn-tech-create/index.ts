/**
 * technician/efn-tech-create/index.ts
 */
...
import { createTechnician }               from "./handler.ts";
import { CreateTechnicianSchema }         from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body = await parseBody(req, CreateTechnicianSchema, correlationId);
    ...
    log.info({ correlationId, tech_id: result.technician_id, duration_ms: log.elapsed() }, "Technician created");
    return respond.created(result, result.technician_id);
  } catch (err) {
