/**
 * technician/efn-tech-workload/index.ts
 */
...
import { getTechWorkload }                from "./handler.ts";
import { GetWorkloadSchema }              from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body = await parseBody(req, GetWorkloadSchema, correlationId);

    const result = await getTechWorkload(
