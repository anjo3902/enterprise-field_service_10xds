/**
 * inventory/efn-inventory-reservation/index.ts
 */
...
import { handleInventoryReservation }     from "./handler.ts";
import { InventoryReservationSchema }     from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body = await parseBody(req, InventoryReservationSchema, correlationId);

    const result = await handleInventoryReservation(
      body, ctx.claims, correlationId,
      req.headers.get("x-forwarded-for") ?? undefined,
      req.headers.get("user-agent") ?? undefined,
    );

    return body.action === "reserve" ? respond.created(result, result.reservation_id) : respond.ok(result);
  } catch (err) {
