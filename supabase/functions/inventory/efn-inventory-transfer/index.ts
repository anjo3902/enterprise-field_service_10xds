/**
 * inventory/efn-inventory-transfer/index.ts
 */
...
import { handleInventoryTransfer }        from "./handler.ts";
import { InventoryTransferSchema }        from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, [
      ...
      "warehouse_manager", "inventory_manager", "vendor_admin", "vendor_supervisor", "technician"
    ], correlationId);
    
    const body = await parseBody(req, InventoryTransferSchema, correlationId);

    const result = await handleInventoryTransfer(
