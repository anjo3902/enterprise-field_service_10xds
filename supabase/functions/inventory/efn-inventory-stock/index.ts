/**
 * inventory/efn-inventory-stock/index.ts
 */
...
import { handleInventoryStock }           from "./handler.ts";
import { InventoryStockSchema }           from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, [
      ...
      "technician", "read_only", "support_engineer", "platform_operator"
    ], correlationId);
    
    const body = await parseBody(req, InventoryStockSchema, correlationId);

    const result = await handleInventoryStock(
