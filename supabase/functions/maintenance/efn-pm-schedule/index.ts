/**
 * maintenance/efn-pm-schedule/index.ts
 */
...
import { handlePmSchedule }               from "./handler.ts";
import { PmScheduleSchema }               from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, [
      "system_admin", "org_admin", "org_manager", "maintenance_manager", "vendor_admin", "dispatcher"
    ], correlationId);
    
    const body = await parseBody(req, PmScheduleSchema, correlationId);

    const result = await handlePmSchedule(
      body, ctx.claims, correlationId,
      req.headers.get("x-forwarded-for") ?? undefined,
      req.headers.get("user-agent") ?? undefined,
    );

    return body.action === "generate" ? respond.created(result) : respond.ok(result);
  } catch (err) {
