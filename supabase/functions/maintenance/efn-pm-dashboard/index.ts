/**
 * maintenance/efn-pm-dashboard/index.ts
 */
...
import { getPmDashboard }                 from "./handler.ts";
import { PmDashboardSchema }              from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, [
      "system_admin", "org_admin", "org_manager", "maintenance_manager", "vendor_admin", "vendor_supervisor", "read_only"
    ], correlationId);
    
    const body = await parseBody(req, PmDashboardSchema, correlationId);

    const result = await getPmDashboard(body, ctx.claims, correlationId);
