/**
 * maintenance/efn-warranty-management/index.ts
 */
...
import { handleWarrantyManagement }       from "./handler.ts";
import { WarrantyManagementSchema }       from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body = await parseBody(req, WarrantyManagementSchema, correlationId);

    const result = await handleWarrantyManagement(
