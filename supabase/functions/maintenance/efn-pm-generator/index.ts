/**
 * maintenance/efn-pm-generator/index.ts
 */
...
import { handlePmGenerator }              from "./handler.ts";
import { PmGeneratorSchema }              from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body = await parseBody(req, PmGeneratorSchema, correlationId);

    const result = await handlePmGenerator(
