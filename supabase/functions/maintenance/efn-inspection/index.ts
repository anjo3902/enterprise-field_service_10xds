/**
 * maintenance/efn-inspection/index.ts
 */
...
import { handleInspection }               from "./handler.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, [
      "system_admin", "org_admin", "org_manager", "maintenance_manager", "vendor_admin", "vendor_supervisor", "technician"
    ], correlationId);
    
    const body = await parseBody(req, InspectionSchema, correlationId);

    const result = await handleInspection(
      body, ctx.claims, correlationId,
      req.headers.get("x-forwarded-for") ?? undefined,
      req.headers.get("user-agent") ?? undefined,
    );

    return body.action === "create_template" ? respond.created(result) : respond.ok(result);
  } catch (err) {
