/**
 * ticket/efn-ticket-search/index.ts
 */
...
import { searchTickets }                  from "./handler.ts";
import { TicketSearchSchema }             from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, [
      ...
      "vendor_admin", "vendor_supervisor", "dispatcher", "technician",
      "read_only", "support_engineer", "platform_operator"
    ], correlationId);
    const body = await parseBody(req, TicketSearchSchema, correlationId);

    const result = await searchTickets(body, ctx.claims, correlationId);
