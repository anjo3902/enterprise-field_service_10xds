/**
 * ticket/efn-ticket-watchers/index.ts
 */
...
import { handleWatcher }                  from "./handler.ts";
import { WatcherActionSchema }            from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, [
      ...
      "vendor_admin", "vendor_supervisor", "dispatcher", "support_engineer"
    ], correlationId);
    const body = await parseBody(req, WatcherActionSchema, correlationId);

    const result = await handleWatcher(body, ctx.claims, correlationId);
