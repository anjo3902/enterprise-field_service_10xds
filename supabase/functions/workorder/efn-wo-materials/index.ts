/**
 * workorder/efn-wo-materials/index.ts
 */
...
import { handleMaterial }                 from "./handler.ts";
import { MaterialActionSchema }           from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body = await parseBody(req, MaterialActionSchema, correlationId);

    const result = await handleMaterial(
