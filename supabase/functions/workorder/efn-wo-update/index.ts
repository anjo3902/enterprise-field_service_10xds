/**
 * workorder/efn-wo-update/index.ts
 */
...
import { updateWorkOrder }                from "./handler.ts";
import { UpdateWorkOrderSchema }          from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, [
      ...
      "dispatcher", "vendor_admin", "vendor_supervisor", "technician", "support_engineer"
    ], correlationId);
    const body = await parseBody(req, UpdateWorkOrderSchema, correlationId);
