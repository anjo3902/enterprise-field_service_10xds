/**
 * workorder/efn-wo-history/index.ts
 */
...
import { getWoHistory }                   from "./handler.ts";
import { WoHistorySchema }                from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body = await parseBody(req, WoHistorySchema, correlationId);

    const result = await getWoHistory(body, ctx.claims, correlationId);
