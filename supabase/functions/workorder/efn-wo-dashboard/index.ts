/**
 * workorder/efn-wo-dashboard/index.ts
 */
...
import { getWoDashboard }                 from "./handler.ts";
import { WoDashboardSchema }              from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, [
      ...
      "dispatcher", "read_only", "support_engineer", "platform_operator"
    ], correlationId);
    const body = await parseBody(req, WoDashboardSchema, correlationId);

    const result = await getWoDashboard(body, ctx.claims, correlationId);
