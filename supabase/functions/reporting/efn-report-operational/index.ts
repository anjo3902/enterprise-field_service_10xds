/**
 * reporting/efn-report-operational/index.ts
 */
...
import { handleOperationalReport }        from "./handler.ts";
import { OperationalReportSchema }        from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, [
      "system_admin", "org_admin", "org_manager", "vendor_admin", "maintenance_manager", "dispatcher", "read_only"
    ], correlationId);
    
    const body = await parseBody(req, OperationalReportSchema, correlationId);

    const result = await handleOperationalReport(body, ctx.claims, correlationId);
