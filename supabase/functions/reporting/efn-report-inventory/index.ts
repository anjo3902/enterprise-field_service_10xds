/**
 * reporting/efn-report-inventory/index.ts
 */
...
import { handleInventoryReport }          from "./handler.ts";
import { InventoryReportSchema }          from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, [
      "system_admin", "org_admin", "org_manager", "vendor_admin", "maintenance_manager", "read_only"
    ], correlationId);
    
    const body = await parseBody(req, InventoryReportSchema, correlationId);

    const result = await handleInventoryReport(body, ctx.claims, correlationId);
