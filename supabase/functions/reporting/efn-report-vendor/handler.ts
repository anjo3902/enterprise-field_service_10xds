/**
 * reporting/efn-report-vendor/handler.ts
 * ─────────────────────────────────────────────────────────────────
 * Retrieves vendor performance, SLA compliance, and contract metrics.
 */
...
import type { VendorReportResult } from "./types.ts";
import type { VendorReportInput } from "./schema.ts";

const FUNCTION_NAME = "efn-report-vendor";

export async function handleVendorReport(
  body:          VendorReportInput,
  claims:        AppClaims,
  correlationId: string,
): Promise<VendorReportResult> {
  ...
  // If role is vendor_admin, restrict to their vendor
  const vendorId = (claims.app_role === "vendor_admin") ? claims.vendor_id : body.vendor_id;

  let query = db.from("vendor_performance_metrics")
    .select("*", { count: "exact" })
    .eq("org_id", orgId)
  ...
  await db.from("audit_logs").insert({
    ...
    new_value: { report_type: "vendor", filters: body }, timestamp: now
  });

  await publishEvent({ event_name: "report.generated" as never, payload: { report_type: "vendor", filters: body }, org_id: orgId, correlation_id: correlationId, source_function: FUNCTION_NAME });

  log.info({ correlationId, orgId, count }, "Vendor report generated");
