/**
 * reporting/efn-report-financial/handler.ts
 * ─────────────────────────────────────────────────────────────────
 * Retrieves financial metrics (revenue, labor cost, parts cost, profit).
 */
...
import type { FinancialReportResult } from "./types.ts";
import type { FinancialReportInput } from "./schema.ts";

const FUNCTION_NAME = "efn-report-financial";

export async function handleFinancialReport(
  body:          FinancialReportInput,
  claims:        AppClaims,
  ...
  let query = db.from("revenue_analytics")
    .select("*", { count: "exact" })
    .eq("org_id", orgId)
  ...
  await db.from("audit_logs").insert({
    ...
    new_value: { report_type: "financial", filters: body }, timestamp: now
  });

  await publishEvent({ event_name: "report.generated" as never, payload: { report_type: "financial", filters: body }, org_id: orgId, correlation_id: correlationId, source_function: FUNCTION_NAME });

  log.info({ correlationId, orgId, count }, "Financial report generated");
