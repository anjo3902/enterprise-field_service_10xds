/**
 * reporting/efn-report-asset/index.ts
 */
...
import { handleAssetReport }              from "./handler.ts";
import { AssetReportSchema }              from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    assertRole(ctx.claims, [
      "system_admin", "org_admin", "org_manager", "maintenance_manager", "read_only"
    ], correlationId);
    
    const body = await parseBody(req, AssetReportSchema, correlationId);

    const result = await handleAssetReport(body, ctx.claims, correlationId);
