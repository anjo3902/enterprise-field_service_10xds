/**
 * reporting/efn-report-ai/index.ts
 */
...
import { handleAiReport }                 from "./handler.ts";
import { AiReportSchema }                 from "./schema.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body = await parseBody(req, AiReportSchema, correlationId);

    const result = await handleAiReport(body, ctx.claims, correlationId);
