...
def _fetch_jobs(tech_token: str) -> tuple[list[dict], list[dict]]:
    tech_headers = _auth_headers(tech_token)
    ...
    return payload.get('jobs') or [], payload.get('completed_jobs') or []


def _wait_for_job_removed(tech_token: str, job_id: str | int, attempts: int = 6) -> tuple[list[dict], list[dict]]:
    last_active = []
    last_completed = []
    for _ in range(attempts):
        last_active, last_completed = _fetch_jobs(tech_token)
        if all(str(job.get('id')) != str(job_id) for job in last_active):
            break
        time.sleep(1)
    return last_active, last_completed


def test_job_completion_removes_active():
    ...
    job2 = _seed_assigned_job(admin_token, technician_id, test_run_id)

    _start_job(tech_token, job1['id'])
    _complete_job(tech_token, job1['id'])

    active_jobs, completed_jobs = _wait_for_job_removed(tech_token, job1['id'])

    assert all(str(job.get('id')) != str(job1['id']) for job in active_jobs)
    assert any(str(job.get('id')) == str(job1['id']) for job in completed_jobs)
    assert any(str(job.get('id')) == str(job2['id']) for job in active_jobs)
